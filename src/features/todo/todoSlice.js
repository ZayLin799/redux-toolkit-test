...
export const fetchAll = createAsyncThunk("todo/fetchAll", async () => {
...
export const todoSlice = createSlice({
  ...
  reducers: {
    ...
    del: (state, action) => state.filter((item) => item.id !== action.payload),
    toggle: (state, action) => {
      state.map((item) => {
  ...
  extraReducers: (builder) => {
    builder.addCase(fetchAll.fullfilled, (state, action) => {
...
export const selectTasks = (state) =>
  state.todo.filter((item) => item.done === false);
export const selectDone = (state) =>
